...
const FlashcardManager = ({ documentId }) => {
  ...
  const renderFlashcardView = () => {
    ...
    return (
      <div className="p-6 space-y-6">
        ...
        {currentCard ? (
          <Flashcard key={currentCard._id} flashcard={currentCard} onToggleStar={() => handleToggle(currentCard._id)} />
        ) : (
          <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 p-6 text-center text-slate-600 dark:text-slate-300">
        ...
        <div className="flex items-center justify-between gap-3">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 px-4 py-2 text-sm font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700 disabled:opacity-50"
          >
  ...
  const renderSetList = () => {
    ...
    if (flashcardSets.length === 0) {
      return (
        <div className="p-8 flex flex-col items-center text-center gap-3 rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50/70 dark:bg-slate-800/70">
          ...
          <div className="space-y-1">
            <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100">No flashcard sets yet</h3>
            <p className="text-sm text-slate-600 dark:text-slate-400">Generate flashcards from your document to start learning and reinforce your knowledge.</p>
          </div>
          <button
    ...
    return (
      <div className="">
        <div className="mb-4 flex items-center justify-between gap-3">
          <div className='p-4'>
            <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Your Flashcard sets</h3>
            <p className="text-sm text-slate-600 dark:text-slate-400">{flashcardSets.length} sets</p>
          </div>
          <div className='p-4'>
            <button
              onClick={handleGnerateFlashcards}
        ...
        <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3 px-4 mb-4">
          {flashcardSets.map((set) => (
            <div
              ...
              className="group cursor-pointer rounded-3xl border border-emerald-100/70 dark:border-emerald-900/50 bg-gradient-to-br from-emerald-50 via-white to-slate-50 dark:from-slate-900 dark:via-slate-900 dark:to-slate-950 p-5 shadow-sm transition hover:-translate-y-1 hover:shadow-lg dark:shadow-slate-900/40"
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <button
                  ...
                  className="inline-flex h-9 w-9 items-center justify-center rounded-full text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/50"
              ...
              <div className="mt-4 flex items-center justify-between text-sm">
                <span className="inline-flex items-center gap-2 rounded-full bg-emerald-100/70 dark:bg-emerald-950/70 px-3 py-1 text-emerald-700 dark:text-emerald-400">
                  {set.cards.length} {set.cards.length === 1 ? "card" : "cards"}
                </span>
                <span className="text-slate-500 dark:text-slate-400">Tap to review</span>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <>
      <div className='bg-white dark:bg-slate-900 backdrop-blur-xl border border-slate-200/60 dark:border-slate-700/60 rounded-3xl shadow-xl shadow-slate-200/50 dark:shadow-slate-900/40'>
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2">
            <button
              ...
              className="rounded bg-slate-200 dark:bg-slate-700 px-4 py-2 text-sm font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-300 dark:hover:bg-slate-600"
            >
              Cancel
            </button>
            <button
