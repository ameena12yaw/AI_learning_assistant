...
const QuizManager = ({ documentId }) => {
    ...
    const handleGenrerateQuiz = async (e) => {
        ...
        if (!count || count < 1 || count > 50) {
            toast.error('Enter a number of questions between 1 and 50')
            return
        }
        ...
        try {
            const res = await aiService.generateQuiz(documentId, { numQuestions: count })
    ...
    return (
        <div>
        <div className='bg-white dark:bg-slate-900 border border-neutral-200 dark:border-slate-700 rounded-lg p-6'>
            <div className='flex justify-end gap-2 mb-4'>
                <Button onClick={() => { setNumQuestions('5'); setIsGenerateModalOpen(true) }}><Plus className='mr-2 h-4 w-4' /> Generate New Quiz</Button>
            </div>
            {renderQuizContent()}
        </div>
            ...
            <form onSubmit={handleGenrerateQuiz} className="space-y-4">
                <div>
                    ...
                    <input
                        ...
                        className="w-full border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                        required
                    />
                </div>
