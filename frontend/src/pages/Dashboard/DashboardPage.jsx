  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-slate-600 dark:text-slate-400">Track your learning progress and recent activity.</p>
      </div>

      {/* Stats grid */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {stats.map((stat) => (
          <div key={stat.label} className="p-6 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm">
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm">
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200 dark:border-slate-700">
          ...
          <span className="text-xs text-slate-500 dark:text-slate-400">Last 5 items</span>
        </div>
              ...
              return (
                <li key={activity.id} className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-full ${isDoc ? 'bg-blue-50 text-blue-600 dark:bg-blue-950 dark:text-blue-400' : 'bg-emerald-50 text-emerald-600 dark:bg-emerald-950 dark:text-emerald-400'}`}>
                      {isDoc ? <FileBadge2 className="h-5 w-5" /> : <ClipboardCheck className="h-5 w-5" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-slate-900 dark:text-slate-100 truncate">
                        {isDoc ? 'Document:' : 'Quiz:'} <span className="font-normal text-slate-700 dark:text-slate-300">{activity.title}</span>
                      </p>
